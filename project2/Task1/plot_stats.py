...
with open(class_txt_path, 'r') as f:
    ordered_classes = [line.strip() for line in f.readlines() if line.strip()]
...
for split, color in colors.items():
    ...
    total_clips = split_data["total_clips"]
    counts = [split_data["counters"][cat] for cat in ordered_classes]
    ...
    ax.set_title(f'Dataset Statistics ({split} split - {total_clips} total clips)', fontsize=15, fontweight='bold')
    ax.set_xticks(x)
