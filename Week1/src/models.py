...
TRANSCD_DIR = "./src/TransCD/"
MODEL_NAME = "Res_SViT_E4_D4_16"

from TransCD.networks.net import TransCDNet
from TransCD.networks import configs as cfg
...
class TransCDBGS(BaseModel):
    def __init__(
        ...
        self.weights= TRANSCD_DIR + MODEL_NAME + ".pth"
        self.net_cfg=MODEL_NAME
    ...
    def _build_transcd(self):
        ...
        model = torch.load(self.weights, weights_only=True, map_location=self.device)
    ...
    def fit(self, frames: np.ndarray):
        self.bg = np.median(frames, axis=0).astype(np.uint8)
    ...
    @torch.no_grad()
    def predict(self, frame: np.ndarray) -> np.ndarray:
        frame_u8 = self._ensure_uint8(frame)
