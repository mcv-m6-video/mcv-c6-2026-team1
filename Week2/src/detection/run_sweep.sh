...
# 1. Capture the target YAML file from the console
CONFIG_FILE=$1

# Validate that an argument is provided
if [ -z "$CONFIG_FILE" ]; then
    echo "Error: Missing configuration file."
    echo "Usage: src/detection/run_sweep.sh <path_to_yaml>"
    echo "Example: src/detection/run_sweep.sh src/detection/sweep_yolo.yaml"
    exit 1
fi
